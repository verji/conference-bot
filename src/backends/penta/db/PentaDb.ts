...
export class PentaDb {
    ...
    public async findPeopleWithId(personId: string): Promise<IPerson[]> {
        ...
        if (Number.isSafeInteger(numericPersonId)) {
            const result = await this.client.query<IDbPerson>(`${PEOPLE_SELECT} ${config.conference.schedule.database?.tblPeople} WHERE person_id = $1 OR person_id = $2`, [personId, numericPersonId]);
            return this.sanitizeRecords(result.rows).map(dbPersonToPerson);
        } else {
            const result = await this.client.query<IDbPerson>(`${PEOPLE_SELECT} ${config.conference.schedule.database?.tblPeople} WHERE person_id = $1`, [personId]);
            return this.sanitizeRecords(result.rows).map(dbPersonToPerson);
        }
    }

    public async findAllPeople(): Promise<IPerson[]> {
        const result = await this.client.query<IDbPerson>(`${PEOPLE_SELECT} ${config.conference.schedule.database?.tblPeople}`);
        return this.sanitizeRecords(result.rows).map(dbPersonToPerson);
    }

    public async findAllPeopleForAuditorium(auditoriumId: string): Promise<IPerson[]> {
        const result = await this.client.query<IDbPerson>(`${NONEVENT_PEOPLE_SELECT} ${config.conference.schedule.database?.tblPeople} ${config.conference.schedule.database?.tblPeople} WHERE conference_room = $1`, [auditoriumId]);
        return this.sanitizeRecords(result.rows).map(dbPersonToPerson);
    }

    public async findAllPeopleForTalk(talkId: string): Promise<IPerson[]> {
        const result = await this.client.query<IDbPerson>(`${PEOPLE_SELECT} ${config.conference.schedule.database?.tblPeople} WHERE event_id = $1`, [talkId]);
        return this.sanitizeRecords(result.rows).map(dbPersonToPerson);
    }

    public async findAllPeopleWithRole(role: Role): Promise<IPerson[]> {
        const result = await this.client.query<IDbPerson>(`${PEOPLE_SELECT} ${config.conference.schedule.database?.tblPeople} WHERE event_role = $1`, [role]);
        return this.sanitizeRecords(result.rows).map(dbPersonToPerson);
    }

    public async findAllPeopleWithRemark(remark: string): Promise<IPerson[]> {
        const result = await this.client.query<IDbPerson>(`${PEOPLE_SELECT} ${config.conference.schedule.database?.tblPeople} WHERE remark = $1`, [remark]);
        return this.sanitizeRecords(result.rows).map(dbPersonToPerson);
    }
    ...
    public async getTalk(talkId: string): Promise<IDbTalk | null> {
        const result = await this.client.query(
            `${SCHEDULE_SELECT} ${config.conference.schedule.database?.tblSchedule} WHERE event_id::text = $2`,
    ...
    private async getTalksWithin(timeQuery: string, inNextMinutes: number, minBefore: number): Promise<IDbTalk[]> {
        ...
        const result = await this.client.query(
            `${SCHEDULE_SELECT} ${config.conference.schedule.database?.tblSchedule} WHERE ${timeQuery} >= (${now} - MAKE_INTERVAL(mins => $2)) AND ${timeQuery} <= (${now} + MAKE_INTERVAL(mins => $3))`,
